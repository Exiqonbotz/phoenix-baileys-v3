...
export const generateWAMessageContent = async (
	...
	if (isInteractiveButtonsMessage) {
		...
		let hasMediaAttachment =
			typeof message.hasMediaAttachment === 'boolean' ? message.hasMediaAttachment : false
		const headerContent: proto.Message.InteractiveMessage.IHeader = {
			title: message.title || '',
		...
		if (message.image) {
			...
			headerContent.imageMessage = imageMessage
			hasMediaAttachment =
				typeof message.hasMediaAttachment === 'boolean' ? message.hasMediaAttachment : true
			headerContent.hasMediaAttachment = hasMediaAttachment
		} else if (message.video) {
			const { videoMessage } = await prepareWAMessageMedia({ video: message.video }, options)
			headerContent.videoMessage = videoMessage
			hasMediaAttachment =
				typeof message.hasMediaAttachment === 'boolean' ? message.hasMediaAttachment : true
			headerContent.hasMediaAttachment = hasMediaAttachment
		}

		const interactiveContextInfo: proto.IContextInfo = {
			...(message.contextInfo || {})
		}
		if (message.mentions?.length) {
			interactiveContextInfo.mentionedJid = message.mentions
		}
		if (message.mentionAll) {
			interactiveContextInfo.nonJidMentions = 1
		}
		...
		const interactiveMessage = proto.Message.InteractiveMessage.create({
			...
			contextInfo: Object.keys(interactiveContextInfo).length ? interactiveContextInfo : undefined
		})
